...
function flattenOptions(options, baseOptions) {
    ...
    var opts = __assign(__assign({ _useHtmlParser2: !!options.xmlMode }, baseOptions), options);
    if (options.xml) {
...
function html$2(dom, options) {
     ...
     */
    var opts = __assign(__assign({}, this === null || this === void 0 ? void 0 : this._options), flattenOptions(options));
...
function text$2(elements) {
    var elems = elements !== null && elements !== void 0 ? elements : (this ? this.root() : []);
    var ret = '';
...
function parseHTML$1(data, context, keepScripts) {
    ...
    var parsed = this.load(data, this._options, false);
    if (!keepScripts) {
     ...
     */
    return __spreadArray([], parsed.root()[0].children, true);
}
/**
...
function attr(name, value) {
    ...
    if (typeof name === 'object' || value !== undefined) {
        if (typeof value === 'function') {
            ...
            return domEach(this, function (el, i) {
                if (isTag(el))
                    setAttr(el, name, value.call(el, i, el.attribs[name]));
            });
        }
        return domEach(this, function (el) {
            if (!isTag(el))
                return;
...
function readAllData(el) {
    for (var _i = 0, _a = Object.keys(el.attribs); _i < _a.length; _i++) {
        ...
        var jsName = camelCase(domName.slice(dataAttrPrefix.length));
...
function readData(el, name) {
    var domName = dataAttrPrefix + cssCase(name);
    var data = el.data;
    ...
    switch (element.name) {
        ...
        case 'select': {
            ...
            return this.attr('multiple')
                ? option.toArray().map(function (el) { return text$2(el.children); })
                : option.attr('value');
        }
        case 'input':
        case 'option': {
...
function hasClass(className) {
    return this.toArray().some(function (elem) {
        var clazz = isTag(elem) && elem.attribs['class'];
        var idx = -1;
...
var getNCheck = unwrapExports(lib);
lib.compile;
lib.parse;
...
function some(elements, selector, options = {}) {
    ...
    const [plain, filtered] = groupSelectors(parse$3(selector));
...
function filter$1(selector, elements, options = {}) {
    return filterParsed(parse$3(selector), elements, options);
}
/**
...
function find(selectorOrHaystack) {
    ...
    if (typeof selectorOrHaystack !== 'string') {
        var haystack = isCheerio(selectorOrHaystack)
            ? selectorOrHaystack.toArray()
            : [selectorOrHaystack];
        var context_1 = this.toArray();
        return this._make(haystack.filter(function (elem) { return context_1.some(function (node) { return contains$1(node, elem); }); }));
    }
    ...
    return function (selector, filterSelector) {
        ...
        matches =
            typeof selector === 'string'
                ? function (elem) { return is$1(elem, selector, _this.options); }
                : selector
 ...
 */
var parent = _singleMatcher(function (_a) {
 ...
 */
var parentsUntil = _matchUntil(function (_a) {
    var parent = _a.parent;
    return (parent && !isDocument(parent) ? parent : null);
}, uniqueSort, function (elems) { return elems.reverse(); });
/**
...
function closest(selector) {
    ...
    var selectFn = typeof selector === 'string'
        ? function (elem) { return is$1(elem, selector, selectOpts); }
        : getFilterFn(selector);
    domEach(this, function (elem) {
...
var siblings = _matcher(function (elem) {
    return getSiblings(elem).filter(function (el) { return isTag(el) && el !== elem; });
}, uniqueSort);
/**
...
function contents() {
    var elems = this.toArray().reduce(function (newElems, elem) {
        return hasChildren(elem) ? newElems.concat(elem.children) : newElems;
    }, []);
...
function filterArray(nodes, match, xmlMode, root) {
    return typeof match === 'string'
        ? filter$1(match, nodes, { xmlMode: xmlMode, root: root })
...
function not(match) {
    ...
    if (typeof match === 'string') {
        var matches_1 = new Set(filter$1(match, nodes, this.options));
...
function add(other, context) {
    ...
    var contents = uniqueSort(__spreadArray(__spreadArray([], this.get(), true), selection.get(), true));
...
function appendTo(target) {
    var appendTarget = isCheerio(target) ? target : this._make(target);
...
function prependTo(target) {
    var prependTarget = isCheerio(target) ? target : this._make(target);
...
function text$1(str) {
    ...
    if (typeof str === 'function') {
        // Function support
        return domEach(this, function (el, i) {
            return _this._make(el).text(str.call(el, i, text$2([el])));
        });
    }
    ...
    return domEach(this, function (el) {
        ...
        var textNode = new Text("".concat(str));
        update(textNode, el);
    });
}
/**
...
function clone() {
    var clone = Array.prototype.map.call(this.get(), function (el) {
        return cloneNode(el, true);
    });
    // Add a root node around the cloned nodes
    var root = new Document(clone);
...
function getLoad(parse, render) {
    ...
    return function load(content, options, isDocument) {
        ...
        var internalOpts = flattenOptions(options);
        ...
        function initialize(selector, context, root, opts) {
            ...
            if (selector && isCheerio(selector))
                return selector;
            var options = flattenOptions(opts, internalOpts);
            var r = typeof root === 'string'
                ? [parse(root, options, false, null)]
                : 'length' in root
                    ? root
                    : [root];
            var rootInstance = isCheerio(r)
                ? r
            ...
            var elements = typeof selector === 'string' && isHtml(selector)
                ? // $(<html>)
...
decode.decodeHTMLAttribute;
decode.decodeHTML;
var decode_5 = decode.determineBranch;
var decode_6 = decode.EntityDecoder;
var decode_7 = decode.DecodingMode;
var decode_8 = decode.BinTrieFlags;
var decode_9 = decode.fromCodePoint;
decode.replaceCodePoint;
decode.decodeCodePoint;
var decode_12 = decode.xmlDecodeTree;
var decode_13 = decode.htmlDecodeTree;
...
let Tokenizer$1 = class Tokenizer {
    ...
    _matchNamedCharacterReference(cp) {
        ...
        for (let i = 0, current = decode_13[0]; i >= 0; cp = this._consume()) {
            i = decode_5(decode_13, current, i + 1, cp);
            if (i < 0)
                break;
            excess += 1;
            current = decode_13[i];
            const masked = current & decode_8.VALUE_LENGTH;
            // If the branch is a value, store it and continue
            if (masked) {
                // The mask is the number of bytes of the value, including the current byte.
                ...
                if (cp !== CODE_POINTS.SEMICOLON &&
                    ...
                    result =
                        valueLength === 0
                            ? [decode_13[i] & ~decode_8.VALUE_LENGTH]
                            : valueLength === 1
                                ? [decode_13[++i]]
                                : [decode_13[++i], decode_13[++i]];
                    excess = 0;
...
class Tokenizer {
    constructor({ xmlMode = false, decodeEntities = true, }, cbs) {
        ...
        this.entityDecoder = new decode_6(xmlMode ? decode_12 : decode_13, (cp, consumed) => this.emitCodePoint(cp, consumed));
    }
    reset() {
        this.state = State.Text;
        this.buffer = "";
...
class Parser {
    ...
    ontextentity(cp, endIndex) {
        var _a, _b;
        this.endIndex = endIndex - 1;
        (_b = (_a = this.cbs).ontext) === null || _b === void 0 ? void 0 : _b.call(_a, decode_9(cp));
        this.startIndex = endIndex;
    }
    /**
 ...
 * @category Cheerio
 */
var parse = getParse(function (content, options, isDocument, context) {
    return options._useHtmlParser2
 ...
 * @see {@link https://cheerio.js.org#loading} for additional usage information.
 */
var load = getLoad(parse, function (dom, options) {
    return options._useHtmlParser2
        ? render$1(dom, options)
        : renderWithParse5(dom);
});
var defaultInstance = load([]);
var contains = contains$1, merge = merge$1;
/**
 ...
 * @returns The rendered document.
 */
var html = html$2.bind(defaultInstance);
/**
 * Render the document as XML.
 *
 ...
 * @returns The rendered document.
 */
var xml = xml$1.bind(defaultInstance);
/**
 * Render the document as text.
 *
 * This returns the `textContent` of the passed elements. The result will
 ...
 * @returns The rendered document.
 */
var text = text$2.bind(defaultInstance);
/**
 ...
 * ```
 */
var parseHTML = parseHTML$1.bind(defaultInstance);
/**
 ...
 * ```
 */
var root = root$1.bind(defaultInstance);

export { contains, defaultInstance as default, html, load, merge, parseHTML, root, text, xml };
